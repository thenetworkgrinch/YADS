    ...
    , m_currentMatchNumber(-1)
{
    ...
    m_detectionTimer->setInterval(FMS::DETECTION_INTERVAL_MS);
    connect(m_detectionTimer.get(), &QTimer::timeout, this, &FMSHandler::onDetectionTimerTimeout);
    
    // Setup heartbeat timer
    m_heartbeatTimer->setInterval(FMS::HEARTBEAT_INTERVAL_MS);
...
void FMSHandler::startMonitoring()
{
    ...
    if (!m_detectionSocket->bind(FMS::FMS_DETECTION_PORT, QUdpSocket::ShareAddress)) {
        m_logger->error("FMS Detection", "Failed to bind detection socket", 
                       QString("Port %1: %2").arg(FMS::FMS_DETECTION_PORT).arg(m_detectionSocket->errorString()));
        updateState(Error);
        return;
    }
...
void FMSHandler::onDetectionTimerTimeout()
{
    if (m_currentState == Detecting) {
        ...
        if (timeSinceContact > FMS::CONNECTION_TIMEOUT_MS) {
...
void FMSHandler::detectFMS()
{
    ...
    if (!fmsAddr.isNull()) {
        ...
        for (const QString &addrStr : FMS::COMMON_ADDRESSES) {
            QHostAddress addr(addrStr);
            m_detectionSocket->writeDatagram(packet, addr, FMS::FMS_DETECTION_PORT);
        }
    }
}
...
void FMSHandler::connectToFMS(const QHostAddress &address)
{
    ...
    m_fmsSocket->connectToHost(address, FMS::FMS_DATA_PORT);
}
...
QHostAddress FMSHandler::findFMSAddress()
{
    ...
    for (const QString &addrStr : FMS::COMMON_ADDRESSES) {
        ...
        testSocket.connectToHost(addr, FMS::FMS_DATA_PORT);
